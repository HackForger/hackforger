...
- **Separate latent bug found** (NOT fixed here): `ManagePhasesUpdate`
  (`routers/web/hackforger/phase.go:115-124`) writes `custom_name`
  unconditionally *before* the `UpdatePhaseTime` ended-phase guard. So renaming
  an **ended** phase persists the name in the DB yet returns `403
  该阶段已结束，无法修改`. The management Vue (`if (resp.ok)`) ignores the 403, so
  the management list won't refresh immediately, but a reload shows the saved
  name. Worth a follow-up: either move the name write after the guard, or allow
  renaming ended phases (name is independent of the time window). Filed
  separately from #182.
